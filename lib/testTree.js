...
function mapRenderTree (renderTree, stubTree, refCollector, elementId) {
  ...
  function processNode (node) {
    ...
    if (!newProps[constants.ID_KEY]) {
      newProps[constants.ID_KEY] = elementId[0];
      elementId[0]++;
    }

    // Process children
    newProps[constants.CHILD_IDS_KEY] = [];
    var mapChild = function (child) {
      var newChild = processNode(child);
      if (newChild && newChild.props) {
        newProps[constants.CHILD_IDS_KEY].push(newChild.props[constants.ID_KEY]);
      }
      return newChild;
    };

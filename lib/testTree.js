...
function renderProxy () {
  ...
  var processedTree = processNode(renderTree)
  ...
  function processNode (node, index) {
    ...
    if (!_.isUndefined(node.key) && !_.isNull(node.key)) {
    ...
    // Create new node
    var newNode
    if (_.has(node.props, 'children')) {
      newNode = React.createElement(toClone.type, newProps, newChildren)
    } else {
      newNode = React.createElement(toClone.type, newProps)
    }

    // Hackily reset the owner, otherwise refs don't get assigned correctly
    newNode = _.omit(newNode, '_owner')
    newNode._owner = toClone._owner
